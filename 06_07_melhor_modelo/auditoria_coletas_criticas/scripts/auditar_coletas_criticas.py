...
def write_report(
    ...
    c15_c16_decision = (
        ...
        if duplicate_ok
        else "- `C15/C16`: duplicacao resolvida; a C16 foi substituida e agora possui sinais diferentes da C15."
    )
    c16_sensitivity_text = (
        "Retirar apenas C16 quase nao altera o resultado, portanto uma das duplicatas pode ser removida com seguranca."
        if duplicate_ok
        else "C16 nao e mais duplicada; sua remocao agora serve apenas como teste de sensibilidade e nao como recomendacao de limpeza."
    )
    ...
    lines.extend(
        [
            "",
            f"{c16_sensitivity_text} Retirar C17 testa se seu ruido prejudica o treino. Retirar C29 e C31 mede a dependencia das coletas saudaveis do dia 13 presentes no treino, enquanto C28 e C30, tambem do dia 13, estao no teste. Isso mostra dependencia de representacao por dia/condicao e exige validacao deixando um dia inteiro de fora.",
            "",
    ...
    without_pressure = ablations.loc[
        ablations["cenario"].eq("recompensado_sem_pressao")
    ].iloc[0]
    without_soil = ablations.loc[
        ablations["cenario"].eq("recompensado_sem_soil")
    ].iloc[0]
    without_temp = ablations.loc[
        ablations["cenario"].eq("recompensado_sem_temp")
    ].iloc[0]
    lines.extend(
        [
            "",
            f"A variante recompensada sem pressao chegou a `{without_pressure['accuracy']:.4f}` neste split fixo, contra `{base['accuracy']:.4f}` na baseline atual. Isso nao deve ser anunciado como novo resultado final antes de validacao por coleta e por dia. Sem Soil, a accuracy foi `{without_soil['accuracy']:.4f}`; sem temperatura, `{without_temp['accuracy']:.4f}`.",
            "",
...
def main() -> None:
    ...
    pd.DataFrame(
        [
            {"C": "C15", "decisao": "manter" if not duplicate_ok else "manter uma unica representante", "motivo": "sinal distinto apos correcao da C16" if not duplicate_ok else "duplicada exata de C16"},
            {"C": "C16", "decisao": "manter e auditar" if not duplicate_ok else "remover como duplicata", "motivo": "dados corretos substituidos" if not duplicate_ok else "duplicada exata de C15"},
