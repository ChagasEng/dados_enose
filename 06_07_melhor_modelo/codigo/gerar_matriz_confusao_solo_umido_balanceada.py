"""Gera a matriz de confusao balanceada para a condicao de solo umido."""
...
NOME_ARQUIVO = "matriz_confusao_solo_umido_balanceada"
MATRIZ_ESPERADA = [[1059, 212], [746, 525]]


def main() -> None:
    dados = pd.read_csv(ENTRADA)
    umido = dados.loc[dados["faixa_soil"] == "molhado"].copy()
    matriz = confusion_matrix(umido["Classe"], umido["predito"], labels=[0, 1])
    ...
    eixo.set_title("Melhor modelo - solo úmido\n(classes balanceadas)")
