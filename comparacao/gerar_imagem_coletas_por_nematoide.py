...
OUT = ROOT / "comparacao" / "coletas_nematoide"
...
def plot_dataset(df: pd.DataFrame, segments: pd.DataFrame, dataset_name: str, title: str) -> None:
    ...
    fig.tight_layout()
    fig.savefig(OUT / f"{dataset_name}_curvas_coletas_por_nematoide.png", dpi=180, bbox_inches="tight")
    plt.close(fig)
...
def main() -> None:
    datasets = [
        (
            ...
            "Antes do dia 20 - coletas demarcadas por presenca de nematoide",
        ),
        (
            "dia_20_mais",
            ROOT / "comparacao" / "datasets_com_ambiente" / "dia_20_mais_com_ambiente.csv",
            "Dia 20 em diante - coletas demarcadas por presenca de nematoide",
        ),
    ]
    all_segments = []
    for dataset_name, path, title in datasets:
        ...
        segments.to_csv(OUT / f"mapa_coletas_nematoide_{dataset_name}.csv", index=False)
        plot_dataset(df, segments, dataset_name, title)
        all_segments.append(segments)

    pd.concat(all_segments, ignore_index=True).to_csv(
        OUT / "mapa_coletas_nematoide_todos.csv", index=False
    )
