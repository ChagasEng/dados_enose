...
def main() -> None:
    ...
    ).to_csv(MATRICES_DIR / "matriz_confusao_naive_bayes.csv")

    pd.DataFrame(
        normalized_matrix,
        index=["real_0_doente", "real_1_saudavel"],
        columns=["previsto_0_doente", "previsto_1_saudavel"],
    ).to_csv(MATRICES_DIR / "matriz_confusao_naive_bayes_normalizada.csv")
    ...
    display.plot(ax=ax, cmap="Reds", values_format="d", colorbar=False)
    ax.set_title("Matriz de confusao - Naive Bayes")
    ax.set_xlabel("Classe prevista")
    ax.set_ylabel("Classe real")
    fig.tight_layout()
    fig.savefig(MATRICES_DIR / "matriz_confusao_naive_bayes.png", dpi=180)
    plt.close(fig)
    ...
    (REPORTS_DIR / "relatorio_classificacao_naive_bayes.txt").write_text(
    ...
    pd.DataFrame(
        ...
        IMPORTANCES_DIR / "importancia_features_naive_bayes.csv", index=False
    )

    metrics = {
        "modelo": "GaussianNB + PowerTransformer",
    ...
    (METRICS_DIR / "metricas_naive_bayes.json").write_text(
