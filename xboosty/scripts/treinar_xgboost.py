...
def main() -> None:
    ...
    ).to_csv(MATRICES_DIR / "matriz_confusao_xgboost.csv")

    pd.DataFrame(
        normalized_matrix,
        index=["real_0_doente", "real_1_saudavel"],
        columns=["previsto_0_doente", "previsto_1_saudavel"],
    ).to_csv(MATRICES_DIR / "matriz_confusao_xgboost_normalizada.csv")
    ...
    display.plot(ax=ax, cmap="Oranges", values_format="d", colorbar=False)
    ax.set_title("Matriz de confusao - XGBoost")
    ax.set_xlabel("Classe prevista")
    ax.set_ylabel("Classe real")
    fig.tight_layout()
    fig.savefig(MATRICES_DIR / "matriz_confusao_xgboost.png", dpi=180)
    plt.close(fig)
    ...
    (REPORTS_DIR / "relatorio_classificacao_xgboost.txt").write_text(
    ...
    pd.DataFrame(
        ...
        IMPORTANCES_DIR / "importancia_features_xgboost.csv", index=False
    )
    ...
    (METRICS_DIR / "metricas_xgboost.json").write_text(
    ...
    joblib.dump(
        ...
        MODELS_DIR / "modelo_xgboost.joblib",
    )
